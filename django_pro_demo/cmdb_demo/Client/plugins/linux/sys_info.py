...
# 安装包模块
cmd_list = ['yum install redhat-lsb', 'yum install -y net-tools.x86_64']
for cmd in cmd_list:
    os.system(cmd)
...
def get_cpu_info():
    ...
    if len(cpu_model) > 1:
        data['cpu_model'] = cpu_model[1].split()
    else:
        data['cpu_model'] = -1
...
def get_ram_info():
    ...
    # print('yyyyyyyyyyyyyyyyyy',raw_ram_list)
    ram_list = []
    for item in raw_ram_list:
        # print("xxxxxxxxxxxxxx", item)
        item_ram_size = 0
        ram_item_to_dic = {}
        for i in item:
            data = i.split(":")
            if len(data) == 2:
                ...
                if key == "Size":
                    if v.strip() != "No Module Installed":
                        ...
                        item_ram_size = round(float(v.split()[0].strip()))
                    else:
...
def get_nic_info():
    """获取网卡信息
    需要安装yum install -y net-tools.x86_64
    """
...
def get_disk_info():
    ...
    centos 需要安装hdparm,或smartctl
    :return:
    """
    raw_data = subprocess.Popen('sudo smartctl -a /dev/sda |grep Vendor|cut -d: -f2 ', stdout=subprocess.PIPE,
                                shell=True)
    raw_data = raw_data.stdout.read().decode()
    model = raw_data.strip().strip(',')
    sn = ""
